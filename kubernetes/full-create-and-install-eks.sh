...
check_dependencies() {
    ...
    kubectl version --client --short || echo "kubectl: unknown"
...
validate_inputs() {
    ...
    local quota_check
    local current_usage
    quota_check=$(aws service-quotas get-service-quota --service-code ec2 --quota-code L-1216C47A --query 'Quota.Value' --output text 2>/dev/null)
    current_usage=$(aws ec2 describe-instances --filters "Name=instance-state-name,Values=running" --query 'length(Reservations[*].Instances[*])' --output text 2>/dev/null)

    if [[ -n "$quota_check" && -n "$current_usage" ]]; then
        # Convert to integers for comparison
        quota_check=$((quota_check))
        current_usage=$((current_usage))
        if (( current_usage + total_nodes > quota_check )); then
            echo "Warning: Total requested nodes ($total_nodes) may exceed your quota limit ($quota_check)"
...
create_node_group() {
    ...
    case $node_role in
        "standalone-indexer-nodes"|"query-nodes"|"indexer-nodes")
            kubectl get nodes -l eks.amazonaws.com/nodegroup=$group_name -o name | \
                xargs -I '{}' kubectl taint '{}' \
                    aerospike.io/role=$node_role:NoSchedule --overwrite
            ;;
    esac
}
